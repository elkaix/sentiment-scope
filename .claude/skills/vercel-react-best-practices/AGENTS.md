...
8. [Advanced Patterns](#8-advanced-patterns) — **LOW**
   ...
   - 8.3 [Read Promises and Context with use()](#83-read-promises-and-context-with-use)
   - 8.4 [Store Event Handlers in Refs](#84-store-event-handlers-in-refs)
   - 8.5 [useActionState for Form Action State](#85-useactionstate-for-form-action-state)
   - 8.6 [useEffectEvent for Stable Callback Refs](#86-useeffectevent-for-stable-callback-refs)
   - 8.7 [useFormStatus for Pending State in Child Components](#87-useformstatus-for-pending-state-in-child-components)
   - 8.8 [useOptimistic for Instant UI Updates](#88-useoptimistic-for-instant-ui-updates)
...
### 8.3 Read Promises and Context with use()

**Impact: MEDIUM (replaces useEffect data-fetching, allows conditional context reads)**
...
The promise should be created above the `Suspense` boundary (e.g. in a Server Component or a parent that doesn't re-create it on every render) so it isn't recreated each render — see [Strategic Suspense Boundaries](#16-strategic-suspense-boundaries) for the pattern of sharing one promise across multiple `use()` calls.
...
### 8.4 Store Event Handlers in Refs
...
### 8.5 useActionState for Form Action State

**Impact: MEDIUM (removes manual pending/error state and works without client JS)**
...
### 8.6 useEffectEvent for Stable Callback Refs
...
### 8.7 useFormStatus for Pending State in Child Components

**Impact: LOW-MEDIUM (avoids prop-drilling pending state through form children)**
...
### 8.8 useOptimistic for Instant UI Updates

**Impact: MEDIUM (removes hand-rolled optimistic state + rollback logic)**
