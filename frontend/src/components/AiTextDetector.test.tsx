...
it("highlights the submitted text when a detector labels it as AI", async () => {
  vi.mocked(compareAiDetectors).mockResolvedValue({
    results: [item({ label: "ai", confidence: 0.92, scores: { human: 0.08, ai: 0.92 } })],
    disagreement: false,
    warning: WARNING,
  });
  ...
  render(<AiTextDetector />);
  await user.clear(screen.getByRole("textbox"));
  await user.type(screen.getByRole("textbox"), "This paragraph was generated by a model.");
  await user.click(screen.getByRole("button", { name: /detect ai text/i }));

  const highlight = await screen.findByRole("region", { name: /ai-likely highlighted text/i });
  expect(within(highlight).getByText("AI-likely text")).toBeInTheDocument();
  expect(within(highlight).getByText("92.0% confidence")).toBeInTheDocument();
  expect(within(highlight).getByText("This paragraph was generated by a model.")).toBeInTheDocument();
});
